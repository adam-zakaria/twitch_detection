import twitch_detections.cliptu.utils as cliptu
...
@log_calls
def detect(input_video_path, roi, output_folder, timestamps=None):
    """
...
def filter(input_detections_path, output_filtered_path):
  """
  Filters detections to remove duplicates and ensure a minimum time gap between detections.
  """
  print("Filtering")
...
if __name__ == "__main__":
    ...
    for streamer_path in utils.ls('twitch_streams'):
        ...
        # input_video_paths = [p for p in utils.ls(streamer_path) if p.endswith(".mp4") and not p.endswith(".temp.mp4")] # filter .mp4s, avoiding .temp.mp4
